...
class SubjectMaterialModel extends Model
{
     ...
     * @param int|null   $yearId     Filter tahun ajaran (null = semua)
     * @param array|null $levels     Filter level kelas (null/empty = semua level).
     *                               level 0 (Semua Level) selalu ikut ditampilkan.
     */
    public function getAllWithHierarchy(int $subjectId, ?int $yearId = null, ?array $levels = null): array
    {
        $builder = $this->select('subject_materials.*, academic_years.year as year_name')
            ->join('academic_years', 'academic_years.id = subject_materials.year_id', 'left')
            ->where('subject_materials.subject_id', $subjectId);

        if ($yearId) {
            $builder->where('subject_materials.year_id', $yearId);
        }

        // Filter level: hanya level yang diampu guru + level 0 (umum/semua level)
        if (!empty($levels)) {
            $levelSet = array_values(array_unique(array_merge($levels, [0])));
            $builder->whereIn('subject_materials.level', $levelSet);
        }
    ...
    public function getPublishStatusForClasses(int $subMatId, array $classes): array
    {
        ...
        foreach ($classes as $c) {
            ...
            $result[$cid] = [
                ...
                'scheduled_at' => $map[$cid]['scheduled_at'] ?? null,
            ];
        }
    ...
    public function publishToClass(
        ...
        // Normalisasi jadwal: kosong/invalid → NULL (tampil segera).
    ...
    /**
     * Normalisasi input jadwal publish.
     * Menerima format 'Y-m-d H:i' (datetime-local) atau 'Y-m-d H:i:s'.
     * Mengembalikan 'Y-m-d H:i:s' atau NULL bila kosong/tidak valid.
     */
    private function normalizeScheduledAt(?string $scheduledAt): ?string
    {
        ...
        // datetime-local memakai 'T' sebagai pemisah (2026-09-05T08:30)
    ...
    public function publishToClasses(
        ...
        foreach ($classIds as $cid) {
            $this->publishToClass($subMatId, (int)$cid, $userId, $academicYearId, $scheduledAt);
        }
    }
    ...
    public function getPublishedForClass(int $classId, int $academicYearId): array
    {
        ...
        // Filter jadwal: hanya tampil bila scheduled_at NULL atau sudah lewat.
    ...
    public function getPublishedByClass(int $classId, int $academicYearId, ?int $subjectId = null): array
    {
        $db       = \Config\Database::connect();
        $classInfo = $db->table('classes')->where('id', $classId)->get()->getRowArray();
        $level    = (int) ($classInfo['level'] ?? 0);

        // Sub materi yang dipublish ke kelas ini (hormati jadwal publish)
        $publishedIds = array_column(
            $db->table('subject_material_publishes')
                ->select('material_id')
                ->where('class_id', $classId)
        ...
        $builder = $db->table('subject_materials sm')
            ->select('sm.*, s.name as subject_name')
            ->join('subjects s', 's.id = sm.subject_id')
            ->whereIn('sm.id', $publishedIds)
            ->where('sm.is_published', 1);

        // Filter per mapel (dipakai halaman progress per mapel agar tidak bercampur)
        if ($subjectId !== null) {
            $builder->where('sm.subject_id', $subjectId);
        }
    ...
    /**
     * Ubah berbagai format URL video menjadi URL embed yang bisa dimuat di iframe.
     * Mendukung YouTube (watch, youtu.be, shorts, live, embed) dan Vimeo.
     */
    public static function toEmbedUrl(string $url): string
    {
        ...
        // ── YouTube ───────────────────────────────────────────────────────
        // Tangkap 11-karakter video ID dari berbagai format:
        //   youtube.com/watch?v=ID   (v bisa di posisi mana saja di query)
        //   youtu.be/ID
        ...
        //   youtube.com/v/ID
        $ytPatterns = [
            '/[?&]v=([A-Za-z0-9_-]{11})/',                    // watch?v=ID atau ...&v=ID
        ...
        if (stripos($url, 'youtube.com') !== false || stripos($url, 'youtu.be') !== false) {
            foreach ($ytPatterns as $pat) {
                if (preg_match($pat, $url, $m)) {
                    // Domain privacy-enhanced + rel=0 (batasi related video ke channel yg sama)
        ...
        // Jika sudah URL embed atau format lain, kembalikan apa adanya
        return $url;
    }

    /**
     * Ekstrak 11-karakter ID video YouTube dari berbagai format URL.
     * Digunakan untuk membuat link fallback "Tonton di YouTube" bila
     * embed gagal (mis. Error 153 atau video melarang embed).
     * Mengembalikan string kosong jika bukan URL YouTube yang valid.
     */
