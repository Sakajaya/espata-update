...
window.MathJax = {
    ...
    startup: {
        typeset: false   // jangan typeset seluruh halaman otomatis — hanya render saat diperlukan
    }
};
</script>
                    ...
                    <select id="atp_id" class="form-select">
                        ...
                        <?php foreach ($atp_list as $atp): ?>
                            <option value="<?= $atp['id'] ?>"
                                ...
                                data-tps="<?= htmlspecialchars(json_encode($atp['tps'] ?? [], JSON_HEX_APOS | JSON_HEX_QUOT | JSON_HEX_TAG | JSON_HEX_AMP | JSON_UNESCAPED_UNICODE), ENT_QUOTES, 'UTF-8') ?>">
...
document.getElementById('btn-generate')?.addEventListener('click', async function() {
    ...
    if (jenis === 'formatif') {
        ...
        // Sumatif: kirim semester dan ATP IDs di semester itu yang TPs-nya dipilih
        const sem = document.getElementById('semester_id').value;
        body.append('semester', sem);
        const semAtpIds = allAtp.filter(a => {
            if (a.semester != sem) return false;
            if (!a.tps || a.tps.length === 0) return true;
            return a.tps.some(tp => tpIds.includes(String(tp.id)));
        }).map(a => a.id);
...
function renderHasil(soal) {
    ...
    /**
     * Escape HTML pada teks. Delimiter LaTeX ($...$, $$...$$, \(...\), \[...\])
     * dibiarkan utuh — MathJax yang akan memindai & merender delimiter ini
     * di dalam elemen ber-class .math-content.
     *
     * PENTING: JANGAN memecah teks dengan <span> di sekitar LaTeX karena
     * memecah delimiter membuat MathJax gagal ("Math input error").
     */
    ...
    function escHtml(s) {
        // Hanya escape < dan > (untuk cegah injeksi tag).
        // & TIDAK di-escape agar entity LaTeX & perintah tetap utuh.
        // " dan ' tidak perlu karena teks ada di dalam elemen, bukan atribut.
    ...
    container.innerHTML = soal.map((s, i) => {
        let html = `<div class="list-group-item">
            <div class="d-flex justify-content-between align-items-start mb-1">
                <div class="fw-bold" style="flex:1">${i+1}. <span class="math-content">${renderMath(s.question)}</span></div>
                <span class="badge bg-secondary ms-2 flex-shrink-0">${tipeLabel[s.type] || s.type}</span>
            </div>`;
            ...
            for (const [key, val] of Object.entries(s.options)) {
                const isCorrect = s.answer && (
                    Array.isArray(s.answer)
                        ? s.answer.includes(key)
                        : String(s.answer).includes(key)
                );
                html += `<div class="${isCorrect ? 'text-success fw-bold' : ''}">
                    ${escHtml(key)}. <span class="math-content">${renderMath(val)}</span>
        ...
        if (s.type === 'benar_salah' || s.type === 'esai') {
            html += `<div class="ms-3 small text-success mt-1">
                <b>Jawaban:</b> <span class="math-content">${renderMath(String(s.answer))}</span>
            </div>`;
        }

        if (s.explanation) {
            html += `<div class="ms-3 small text-muted mt-1">
                <b>Penjelasan:</b> <span class="math-content">${renderMath(s.explanation)}</span>
            </div>`;
        }
    ...
    // Trigger MathJax render setelah HTML diinjeksikan
    if (window.MathJax) {
...
document.getElementById('btn-save')?.addEventListener('click', async function() {
    ...
    const restoreBtn = function() {
        btn.disabled = false;
        btn.innerHTML = '<i class="bi bi-floppy me-1"></i> Simpan ke Bank Soal';
    };

    try {
        const res = await fetch('<?= base_url("admin/administrasi-guru/soal-ai/save-to-bank") ?>', { method: 'POST', body });

        // Tangani respons non-JSON (mis. error server 500) agar pesan tetap jelas
        let data;
        ...
        try { data = JSON.parse(text); } catch (_) {
            ...
            Swal.fire({
                ...
                html: 'Server mengembalikan respons tidak terduga (kemungkinan sesi berakhir atau error server).<br><small>Status: ' + res.status + '</small>',
            });
            return;
        }
        ...
        if (data.success) {
            Swal.fire({
                ...
                text: data.message || 'Soal berhasil ditambahkan ke bank soal.',
                confirmButtonText: 'Lihat Bank Soal',
                showCancelButton: true,
                cancelButtonText: 'Tetap di sini',
            }).then(function(result) {
                if (result.isConfirmed) {
                    window.location.href = '<?= base_url("admin/cbt/banksoal") ?>';
                } else {
                    location.reload();
                }
            });
            ...
            Swal.fire({
                icon: 'error',
                title: 'Gagal Menyimpan',
                text: data.error || 'Terjadi kesalahan yang tidak diketahui.',
            });
        }
    } catch (e) {
        restoreBtn();
        Swal.fire({
            ...
            text: 'Gagal terhubung ke server: ' + e.message,
        });
    }
});
...
function parseAiJson(raw) {
    ...
    // 5b. FIX LaTeX backslash — kritis untuk soal matematika.
    // AI sering menulis "$\frac{1}{2}$" (single backslash). Di JSON, \f = form-feed,
    // \s = invalid escape → LaTeX rusak → "Math input error" di MathJax.
    // Solusi: escape backslash yang BUKAN escape JSON valid menjadi \\.
    s = fixLatexBackslashes(s);
...
/**
 * Escape backslash LaTeX agar valid di JSON.
 *
 * Masalah: perintah LaTeX (\frac \times \tan \beta \nu \rho) berbagi huruf awal
 * dengan escape JSON (\f \t \n \r \b), sehingga whitelist karakter tidak bisa
 * membedakan. Strategi: gandakan backslash tunggal, tapi pertahankan escape JSON
 * asli (\" \\ \/ \uXXXX, dan \n \r \t yang berdiri sendiri sebagai kontrol).
 *
 * Heuristik: jika \ diikuti HURUF lalu HURUF lagi → itu perintah LaTeX
 * (mis. \frac, \times) → gandakan jadi \\. Jika \ diikuti escape JSON tunggal
 * yang sah (mis. \n di akhir/antar kata) → biarkan.
 */
function fixLatexBackslashes(str) {
    let out = '';
    for (let i = 0; i < str.length; i++) {
        ...
        // \\ (sudah ganda) → salin dua-duanya apa adanya
        if (next === '\\') { out += '\\\\'; i++; continue; }

        // Escape JSON non-huruf yang sah → biarkan
        if (next === '"' || next === '/' || next === 'u') {
            out += '\\' + next; i++; continue;
        }

        // Huruf escape JSON (n r t b f) — bisa jadi kontrol ATAU awal perintah LaTeX
        if ('nrtbf'.includes(next)) {
            // Jika huruf berikutnya juga huruf → perintah LaTeX (\frac, \times, \nu jarang)
            if (/[a-zA-Z]/.test(after)) {
                out += '\\\\'; // gandakan, huruf perintah diproses di iterasi berikut
            } else {
                out += '\\' + next; i++; // escape kontrol JSON asli
            }
            continue;
        }

        // Backslash diikuti huruf lain / simbol LaTeX ({ } ( ) [ ] ^ _ , dll) → gandakan
        out += '\\\\';
    }
    return out;
}
