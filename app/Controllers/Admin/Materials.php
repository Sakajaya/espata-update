<?php

namespace App\Controllers\Admin;

use App\Controllers\BaseController;
use App\Models\SubjectMaterialModel;
use App\Models\MaterialProgressModel;
use App\Models\ForumReplyModel;
use App\Models\ForumThreadModel;
use App\Models\SubjectModel;
use App\Models\AcademicYearModel;

/**
 * Admin/Materials — Kelola Materi & Sub Materi
 *
 * Alur baru:
 *   - Materi dikaitkan ke subject_id + level (bukan class_id)
 *   - Publikasi dikelola terpisah via subject_material_publishes
 *   - Guru bisa "gunakan" sub materi milik guru lain (shared reference)
 *
 * Routes:
 *   GET  /admin/materials                          → subjectList
 *   GET  /admin/materials/{subjectId}              → index (hierarki)
 *   GET  /admin/materials/create/{subjectId}       → create
 *   POST /admin/materials/store                    → store
 *   GET  /admin/materials/edit/{id}                → edit
 *   POST /admin/materials/update/{id}              → update
 *   POST /admin/materials/delete/{id}              → delete (AJAX)
 *   GET  /admin/materials/publish/{id}             → publishPage
 *   POST /admin/materials/publish/{id}             → publishSave
 *   GET  /admin/materials/use/{subjectId}          → usePage
 *   POST /admin/materials/use                      → useSave
 *   GET  /admin/materials/progress/{subjectId}     → progress
 *   GET  /admin/materials/progress-detail/{id}     → progressDetail
 */
class Materials extends BaseController
{
    protected SubjectMaterialModel  $mat;
    protected MaterialProgressModel $prog;
    protected SubjectModel          $subjectModel;
    protected AcademicYearModel     $yearModel;
    protected ForumReplyModel       $replyModel;
    protected ForumThreadModel      $threadModel;
    protected $db;

    public function __construct()
    {
        $this->mat          = new SubjectMaterialModel();
        $this->prog         = new MaterialProgressModel();
        $this->subjectModel = new SubjectModel();
        $this->yearModel    = new AcademicYearModel();
        $this->replyModel   = new ForumReplyModel();
        $this->threadModel  = new ForumThreadModel();
        $this->db           = \Config\Database::connect();
    }

    // ── Helpers ──────────────────────────────────────────────────────────
    private function user(): array  { return session()->get('user') ?? []; }
    private function userId(): int  { return (int)($this->user()['id'] ?? 0); }
    private function roleId(): int  { return (int)($this->user()['role_id'] ?? 0); }
    private function isAdmin(): bool { return in_array($this->roleId(), [1, 2]); }

    /**
     * Boleh mengelola (buat/ubah/hapus/publish) materi?
     * Hanya Admin (1) & Guru (3). Kepala Sekolah (2) read-only (hanya lihat).
     */
    private function canManage(): bool { return in_array($this->roleId(), [1, 3], true); }

    /**
     * Tolak akses jika read-only (mis. Kepala Sekolah). Kembalikan RedirectResponse
     * bila ditolak, atau null bila boleh lanjut.
     */
    private function denyIfReadonly()
    {
        if (!$this->canManage()) {
            return redirect()->to('admin/materials')
                ->with('error', 'Anda hanya memiliki akses lihat pada materi pelajaran.');
        }
        return null;
    }

    private function teacherId(): ?int
    {
        $u = $this->user();
        if ($u['role_id'] == 3) {
            $t = $this->db->table('teachers')->where('user_id', $u['id'])->get()->getRowArray();
            return $t ? (int)$t['id'] : null;
        }
        return null;
    }

    private function activeYear(): array { return $this->yearModel->getActiveYear() ?? []; }

    /** Level-level kelas yang diajar guru ini untuk mapel tertentu */
    private function getLevelsForSubject(int $subjectId, int $yearId): array
    {
        $builder = $this->db->table('teaching_assignments ta')
            ->select('c.level')
            ->join('classes c', 'c.id = ta.class_id')
            ->where('ta.subject_id',       $subjectId)
            ->where('ta.academic_year_id', $yearId)
            ->groupBy('c.level')
            ->orderBy('c.level', 'ASC');

        if (!$this->isAdmin()) {
            $tid = $this->teacherId();
            if ($tid) $builder->where('ta.teacher_id', $tid);
        }

        return array_column($builder->get()->getResultArray(), 'level');
    }

    // =========================================================
    // SUBJECT LIST
    // =========================================================
    public function subjectList()
    {
        $year   = $this->activeYear();
        $yearId = (int)($year['id'] ?? 0);

        if ($this->isAdmin()) {
            // Daftar mapel yang diampu (dengan jumlah kelas)
            $subjects = $this->db->table('teaching_assignments ta')
                ->select('s.id, s.name, COUNT(DISTINCT ta.class_id) as total_classes')
                ->join('subjects s', 's.id = ta.subject_id')
                ->where('ta.academic_year_id', $yearId)
                ->groupBy('s.id')
                ->orderBy('s.name', 'ASC')
                ->get()->getResultArray();

            // Hitung sub materi & sub materi terbit per mapel (parent_id NOT NULL)
            foreach ($subjects as &$s) {
                $s = array_merge($s, $this->_materialCounts((int)$s['id'], $yearId, null));
            }
            unset($s);
        } else {
            $tid = $this->teacherId();
            if (!$tid) return redirect()->to('dashboard')->with('error', 'Data guru tidak ditemukan.');

            // Daftar mapel yang diampu guru + jumlah kelas
            $subjects = $this->db->table('teaching_assignments ta')
                ->select('s.id, s.name, COUNT(DISTINCT ta.class_id) as total_classes')
                ->join('subjects s', 's.id = ta.subject_id')
                ->where('ta.teacher_id', $tid)
                ->where('ta.academic_year_id', $yearId)
                ->groupBy('s.id')
                ->orderBy('s.name', 'ASC')
                ->get()->getResultArray();

            // Hitung sub materi berdasarkan level yang diampu guru untuk mapel tsb
            foreach ($subjects as &$s) {
                $levels = $this->getLevelsForSubject((int)$s['id'], $yearId);
                $s = array_merge($s, $this->_materialCounts((int)$s['id'], $yearId, $levels));
            }
            unset($s);
        }

        return view('admin/materials/subject_list', [
            'title'      => 'Materi Pelajaran',
            'subjects'   => $subjects,
            'activeYear' => $year,
        ]);
    }

    /**
     * Hitung jumlah sub materi & sub materi terbit untuk satu mapel.
     * Sub materi = subject_materials dengan parent_id NOT NULL.
     * $levels: null = semua level (admin); array = filter level (guru).
     * "Terbit" = is_published = 1 (materi selesai dibuat/siap dipublikasikan).
     *
     * @return array{total_materi:int, total_published:int}
     */
    private function _materialCounts(int $subjectId, int $yearId, ?array $levels): array
    {
        $base = $this->db->table('subject_materials')
            ->where('subject_id', $subjectId)
            ->where('year_id',    $yearId)
            ->where('parent_id IS NOT NULL');

        if ($levels !== null) {
            if (empty($levels)) {
                return ['total_materi' => 0, 'total_published' => 0];
            }
            // Sub materi cocok level guru, atau level 0 (semua)
            $base->groupStart()
                    ->whereIn('level', $levels)
                    ->orWhere('level', 0)
                 ->groupEnd();
        }

        $total = (clone $base)->countAllResults(false);
        $pub   = (clone $base)->where('is_published', 1)->countAllResults();

        return [
            'total_materi'    => (int) $total,
            'total_published' => (int) $pub,
        ];
    }

    // =========================================================
    // INDEX — hierarki Materi per mapel
    // =========================================================
    public function index(int $subjectId)
    {
        $subject    = $this->subjectModel->find($subjectId);
        $year       = $this->activeYear();
        $yearId     = (int)($year['id'] ?? 0);

        // Guard: guru hanya boleh akses mapel yang dia ampu di tahun aktif
        if (!$this->isAdmin()) {
            $tid = $this->teacherId();
            $canAccess = $tid && $this->db->table('teaching_assignments')
                ->where('teacher_id',       $tid)
                ->where('subject_id',       $subjectId)
                ->where('academic_year_id', $yearId)
                ->countAllResults() > 0;
            if (!$canAccess) {
                return redirect()->to('admin/materials')
                    ->with('error', 'Anda tidak mengampu mata pelajaran ini.');
            }
        }

        // Level yang relevan: guru → hanya level yang diampu; admin → semua level
        $levels = $this->isAdmin() ? null : $this->getLevelsForSubject($subjectId, $yearId);

        // Auto-sync ATP → subject_materials setiap kali halaman dibuka
        // Ini memastikan materi induk selalu sesuai ATP terbaru
        $this->_doSyncFromAtp($subjectId, $yearId);

        $hierarchy  = $this->mat->getAllWithHierarchy($subjectId, $yearId, $levels);

        // Progress summary (hanya sub materi)
        $progSummary = [];
        foreach ($hierarchy as $parent) {
            foreach (($parent['children'] ?? []) as $child) {
                $progSummary[$child['id']] = [
                    'completed'   => $this->db->table('material_progress')
                        ->where('material_id', $child['id'])->where('status', 'completed')->countAllResults(),
                    'in_progress' => $this->db->table('material_progress')
                        ->where('material_id', $child['id'])->where('status', 'in_progress')->countAllResults(),
                ];
            }
        }

        return view('admin/materials/index', [
            'title'       => 'Materi: ' . ($subject['name'] ?? ''),
            'subject'     => $subject,
            'hierarchy'   => $hierarchy,
            'progSummary' => $progSummary,
            'returnUrl'   => $this->request->getGet('return'),
            'activeYear'  => $year,
        ]);
    }

    // =========================================================
    // SYNC FROM ATP — sinkronkan ATP → subject_materials
    // POST /admin/materials/sync-atp/{subjectId}
    // =========================================================
    public function syncFromAtp(int $subjectId)
    {
        if (!$this->canManage()) {
            return $this->response->setJSON(['success' => false, 'message' => 'Akses hanya lihat.']);
        }
        $year   = $this->activeYear();
        $yearId = (int)($year['id'] ?? 0);
        $count  = $this->_doSyncFromAtp($subjectId, $yearId);

        return $this->response->setJSON([
            'success' => true,
            'synced'  => $count,
            'message' => $count > 0
                ? "{$count} materi baru disinkronkan dari ATP."
                : 'Semua materi ATP sudah tersinkronkan.',
        ]);
    }

    /**
     * Core sync: buat baris subject_materials (parent_id=NULL) dari ATP
     * yang belum ada. Materi yang sudah ada (title+subject+year+level sama) dilewati.
     * Return jumlah baris baru yang dibuat.
     */
    private function _doSyncFromAtp(int $subjectId, int $yearId): int
    {
        // Ambil ATP untuk mapel ini beserta level kelasnya
        $atpQuery = $this->db->table('alur_tujuan_pembelajaran atp')
            ->select('atp.id as atp_id, atp.lingkup_materi, atp.semester, c.level as class_level')
            ->join('classes c', 'c.id = atp.class_id', 'left')
            ->where('atp.subject_id', $subjectId);

        // Guru: hanya sinkronkan ATP dari level yang dia ampu
        if (!$this->isAdmin()) {
            $levels = $this->getLevelsForSubject($subjectId, $yearId);
            if (empty($levels)) return 0;
            $atpQuery->whereIn('c.level', $levels);
        }

        $atpRows = $atpQuery
            ->orderBy('c.level', 'ASC')
            ->orderBy('atp.semester', 'ASC')
            ->orderBy('atp.id', 'ASC')
            ->get()->getResultArray();

        if (empty($atpRows)) return 0;

        // Deduplikasi ATP: satu level+semester+lingkup_materi = satu materi induk
        // (hindari duplikat jika ATP sama ada di kelas 7A dan 7B)
        $seen = [];
        $unique = [];
        foreach ($atpRows as $row) {
            $key = $row['class_level'] . '|' . $row['semester'] . '|' . mb_strtolower(trim($row['lingkup_materi']));
            if (!isset($seen[$key])) {
                $seen[$key] = true;
                $unique[]   = $row;
            }
        }

        // Ambil materi induk yang sudah ada untuk mapel+tahun ini
        $existing = $this->db->table('subject_materials')
            ->select('LOWER(TRIM(title)) as title_lower, level, semester')
            ->where('subject_id', $subjectId)
            ->where('year_id',    $yearId)
            ->where('parent_id IS NULL')
            ->get()->getResultArray();

        $existingKeys = [];
        foreach ($existing as $e) {
            $existingKeys[$e['level'] . '|' . $e['semester'] . '|' . $e['title_lower']] = true;
        }

        $created = 0;
        foreach ($unique as $atp) {
            $level    = (int)($atp['class_level'] ?? 0);
            $semester = $atp['semester'];
            $title    = trim($atp['lingkup_materi']);
            $checkKey = $level . '|' . $semester . '|' . mb_strtolower($title);

            if (isset($existingKeys[$checkKey])) continue; // sudah ada

            $this->mat->insert([
                'parent_id'    => null,
                'level'        => $level,
                'subject_id'   => $subjectId,
                'year_id'      => $yearId,
                'semester'     => $semester,
                'title'        => $title,
                'is_published' => 1,    // materi induk dari ATP langsung siap
                'created_by'   => $this->userId(),
                'sort_order'   => 0,
            ]);
            $existingKeys[$checkKey] = true; // tandai agar tidak duplikat dalam loop
            $created++;
        }

        return $created;
    }

    // =========================================================
    // SHOW — Lihat konten sub materi + diskusi (admin/guru)
    // GET /admin/materials/show/{id}
    // =========================================================
    public function show(int $subMatId)
    {
        $subMat = $this->mat->find($subMatId);

        if (!$subMat || empty($subMat['parent_id'])) {
            return redirect()->to('admin/materials')->with('error', 'Sub Materi tidak ditemukan.');
        }

        $parent  = $this->mat->find($subMat['parent_id']);
        $subject = $this->subjectModel->find($subMat['subject_id']);
        $year    = $this->activeYear();

        // Embed video
        if ($subMat['content_type'] === 'video' && !empty($subMat['video_url'])) {
            $subMat['embed_url'] = SubjectMaterialModel::toEmbedUrl($subMat['video_url']);
        }

        // Slide gambar
        $slides = [];
        if (($subMat['content_type'] ?? '') === 'slide') {
            $slides = $this->db->table('subject_material_slides')
                ->where('material_id', $subMatId)
                ->orderBy('sort_order', 'ASC')
                ->orderBy('id', 'ASC')
                ->get()->getResultArray();
        }

        // Sibling sub materi (dalam satu materi induk) untuk navigasi
        $siblings = $this->db->table('subject_materials')
            ->where('parent_id', $subMat['parent_id'])
            ->where('is_published', 1)
            ->orderBy('sort_order', 'ASC')
            ->orderBy('id', 'ASC')
            ->get()->getResultArray();

        $currentIdx = array_search($subMatId, array_column($siblings, 'id'));
        $prevSub    = ($currentIdx !== false && $currentIdx > 0)
                        ? $siblings[$currentIdx - 1] : null;
        $nextSub    = ($currentIdx !== false && $currentIdx < count($siblings) - 1)
                        ? $siblings[$currentIdx + 1] : null;

        // ── DISKUSI ──────────────────────────────────────────
        $thread    = $this->mat->getForumThread($subMatId);
        $replyTree = [];
        $threadId  = null;
        $userId    = $this->userId();

        if ($thread) {
            $threadId    = (int)$thread['id'];
            $flatReplies = $this->replyModel->getByThread($threadId, $userId);
            $replyTree   = ForumReplyModel::buildTree($flatReplies);

            // Tandai sudah dibaca
            $this->db->query(
                'INSERT INTO forum_reads (thread_id, user_id, read_at)
                 VALUES (?, ?, ?)
                 ON DUPLICATE KEY UPDATE read_at = VALUES(read_at)',
                [$threadId, $userId, date('Y-m-d H:i:s')]
            );
        }

        // ── PROGRESS SINGKAT (jumlah siswa selesai) ──────────
        $progStats = [
            'completed'   => $this->db->table('material_progress')
                ->where('material_id', $subMatId)->where('status', 'completed')->countAllResults(),
            'in_progress' => $this->db->table('material_progress')
                ->where('material_id', $subMatId)->where('status', 'in_progress')->countAllResults(),
        ];

        // ── KUIS TERKAIT — kuis mandiri yang dikaitkan ke sub materi ini ──
        $relatedQuizzes = $this->db->table('quiz_configs qc')
            ->select('qc.id, qc.title, qc.is_published, qc.duration, qc.max_attempts,
                      qb.code as bank_code')
            ->join('cbt_question_banks qb', 'qb.id = qc.bank_id', 'left')
            ->where('qc.material_id', $subMatId)
            ->orderBy('qc.id', 'DESC')
            ->get()->getResultArray();

        return view('admin/materials/show', [
            'title'      => esc($subMat['title']),
            'subMat'     => $subMat,
            'parent'     => $parent,
            'subject'    => $subject,
            'activeYear' => $year,
            'thread'     => $thread,
            'threadId'   => $threadId,
            'replyTree'  => $replyTree,
            'isMod'      => true,   // admin/guru selalu mod
            'userId'     => $userId,
            'roleId'     => $this->roleId(),
            'prevSub'    => $prevSub,
            'nextSub'    => $nextSub,
            'currentIdx' => (int)($currentIdx !== false ? $currentIdx : 0),
            'siblings'   => $siblings,
            'progStats'  => $progStats,
            'relatedQuizzes' => $relatedQuizzes,
            'slides'     => $slides,
        ]);
    }

    // =========================================================
    // CREATE — hanya untuk Sub Materi
    // Materi induk dibuat otomatis dari ATP via syncFromAtp()
    // =========================================================
    public function create(int $subjectId)
    {
        if ($r = $this->denyIfReadonly()) return $r;
        $subject  = $this->subjectModel->find($subjectId);
        $year     = $this->activeYear();
        $yearId   = (int)($year['id'] ?? 0);
        $parentId = (int)($this->request->getGet('parent_id') ?? 0);
        $parent   = $parentId ? $this->mat->find($parentId) : null;

        // Hanya sub materi yang boleh dibuat manual
        // Daftar materi induk (dari ATP) sebagai pilihan parent
        $parents = $this->mat->getParentsBySubject($subjectId, $yearId);

        if (empty($parents)) {
            return redirect()->to('admin/materials/' . $subjectId)
                ->with('error', 'Belum ada materi dari ATP. Pastikan ATP sudah diisi terlebih dahulu.');
        }

        return view('admin/materials/create', [
            'title'      => 'Tambah Sub Materi',
            'subject'    => $subject,
            'activeYear' => $year,
            'returnUrl'  => $this->request->getGet('return'),
            'parentId'   => $parentId,
            'parent'     => $parent,
            'parents'    => $parents,
            'levels'     => [],   // level diambil dari parent yang dipilih
        ]);
    }

    // =========================================================
    // STORE
    // =========================================================
    public function store()
    {
        if ($r = $this->denyIfReadonly()) return $r;
        $data     = $this->request->getPost();
        $parentId = !empty($data['parent_id']) ? (int)$data['parent_id'] : null;

        // Materi induk hanya boleh dibuat via sync ATP, tidak manual
        if (!$parentId) {
            return redirect()->back()->withInput()
                ->with('error', 'Materi induk dibuat otomatis dari ATP. Pilih Materi (parent) terlebih dahulu untuk membuat Sub Materi.');
        }

        $filePath = null;

        // Upload PDF
        if (($data['content_type'] ?? '') === 'pdf') {
            $file = $this->request->getFile('file_upload');
            if ($file && $file->isValid() && !$file->hasMoved()) {
                if ($file->getMimeType() !== 'application/pdf') {
                    return redirect()->back()->withInput()->with('error', 'File harus PDF.');
                }
                if ($file->getSize() > 10 * 1024 * 1024) {
                    return redirect()->back()->withInput()->with('error', 'File maks 10 MB.');
                }
                $newName = $file->getRandomName();
                $file->move(FCPATH . 'uploads/materials/', $newName);
                $filePath = $newName;
            }
        }

        // Sub materi mewarisi level & semester dari parent-nya (materi induk dari ATP)
        $level    = (int)($data['level'] ?? 0);
        $semester = $data['semester'] ?? '1';
        if ($parentId) {
            $parentRow = $this->mat->find($parentId);
            if ($parentRow) {
                $level    = (int)($parentRow['level'] ?? $level);
                $semester = $parentRow['semester'] ?? $semester;
            }
        }

        $contentType = $data['content_type'] ?? 'text';
        $isPublished = isset($data['is_published']) ? 1 : 0;

        // Konten: untuk 'html' baca field content_html (HTML mentah dari editor kode),
        // untuk 'text' baca field content (dari CKEditor). Dipisah agar CKEditor tidak
        // meng-encode HTML interaktif (penyebab kode tampil mentah).
        $contentValue = null;
        if ($contentType === 'html') {
            $contentValue = $data['content_html'] ?? ($data['content'] ?? null);
        } elseif ($contentType === 'text') {
            $contentValue = $data['content'] ?? null;
        }

        $insertData = [
            'parent_id'         => $parentId,
            'level'             => $level,
            'subject_id'        => $data['subject_id'],
            'year_id'           => $data['year_id'],
            'semester'          => $semester,
            'title'             => $data['title'],
            'description'       => $data['description'] ?? null,
            'content_type'      => $contentType,
            // 'html' menyimpan HTML mentah (interaktif) tanpa filtering — dirender di iframe sandbox.
            'content'           => $contentValue,
            'file_path'         => $contentType === 'pdf'   ? $filePath                         : null,
            'video_url'         => $contentType === 'video' ? ($data['video_url']      ?? null) : null,
            'external_link'     => $contentType === 'link'  ? ($data['external_link']  ?? null) : null,
            'estimated_minutes' => !empty($data['estimated_minutes']) ? (int)$data['estimated_minutes'] : null,
            'sort_order'        => !empty($data['sort_order']) ? (int)$data['sort_order'] : 0,
            'is_published'      => $isPublished,
            'published_by'      => $isPublished ? $this->userId() : null,
            'published_at'      => $isPublished ? date('Y-m-d H:i:s') : null,
            'created_by'        => $this->userId(),
        ];

        $this->mat->insert($insertData);
        $newId = (int)$this->mat->getInsertID();

        // Slide Gambar: simpan gambar-gambar yang diupload
        if ($contentType === 'slide' && $newId) {
            $this->_handleSlideUploads($newId);
        }

        $msg = $parentId ? 'Sub Materi berhasil ditambahkan.' : 'Materi berhasil ditambahkan.';

        $returnUrl = $this->request->getPost('return');
        return redirect()->to($returnUrl ?: '/admin/materials/' . $data['subject_id'])
                         ->with('success', $msg);
    }

    // =========================================================
    // EDIT
    // =========================================================
    public function edit(int $id)
    {
        if ($r = $this->denyIfReadonly()) return $r;
        $material = $this->mat->find($id);
        if (!$material) return redirect()->to('admin/materials')->with('error', 'Tidak ditemukan.');

        $subject    = $this->subjectModel->find($material['subject_id']);
        $year       = $this->activeYear();
        $yearId     = (int)($year['id'] ?? 0);
        $isSubMat   = !empty($material['parent_id']);

        $levels  = $this->getLevelsForSubject((int)$material['subject_id'], $yearId);
        sort($levels);
        $parents = $isSubMat ? [] : []; // tidak bisa ubah parent pada edit

        // Slide gambar existing (untuk tipe 'slide')
        $slides = $this->db->table('subject_material_slides')
            ->where('material_id', $id)
            ->orderBy('sort_order', 'ASC')
            ->orderBy('id', 'ASC')
            ->get()->getResultArray();

        return view('admin/materials/edit', [
            'title'     => $isSubMat ? 'Edit Sub Materi' : 'Edit Materi',
            'material'  => $material,
            'subject'   => $subject,
            'activeYear'=> $year,
            'levels'    => $levels,
            'isSubMat'  => $isSubMat,
            'slides'    => $slides,
        ]);
    }

    // =========================================================
    // UPDATE
    // =========================================================
    public function update(int $id)
    {
        if ($r = $this->denyIfReadonly()) return $r;
        $material    = $this->mat->find($id);
        if (!$material) return redirect()->to('admin/materials')->with('error', 'Tidak ditemukan.');

        $data        = $this->request->getPost();
        $contentType = $data['content_type'] ?? 'text';
        $filePath    = $material['file_path'];
        $wasPublished= (bool)($material['is_published'] ?? false);
        $nowPublish  = isset($data['is_published']);

        // Upload PDF baru
        if ($contentType === 'pdf') {
            $file = $this->request->getFile('file_upload');
            if ($file && $file->isValid() && !$file->hasMoved()) {
                if ($file->getMimeType() !== 'application/pdf') {
                    return redirect()->back()->withInput()->with('error', 'File harus PDF.');
                }
                if (!empty($material['file_path']) &&
                    file_exists(FCPATH . 'uploads/materials/' . $material['file_path'])) {
                    @unlink(FCPATH . 'uploads/materials/' . $material['file_path']);
                }
                $newName = $file->getRandomName();
                $file->move(FCPATH . 'uploads/materials/', $newName);
                $filePath = $newName;
            }
        } elseif ($material['content_type'] === 'pdf' && $contentType !== 'pdf') {
            if (!empty($material['file_path']) &&
                file_exists(FCPATH . 'uploads/materials/' . $material['file_path'])) {
                @unlink(FCPATH . 'uploads/materials/' . $material['file_path']);
            }
            $filePath = null;
        }

        $updateData = [
            'level'             => (int)($data['level'] ?? $material['level']),
            'semester'          => $data['semester'],
            'title'             => $data['title'],
            'description'       => $data['description'] ?? null,
            'content_type'      => $contentType,
            // 'html' menyimpan HTML mentah (interaktif) tanpa filtering — dirender di iframe sandbox.
            'content'           => $contentType === 'html'
                                     ? ($data['content_html'] ?? ($data['content'] ?? null))
                                     : ($contentType === 'text' ? ($data['content'] ?? null) : null),
            'file_path'         => $contentType === 'pdf'   ? $filePath                        : null,
            'video_url'         => $contentType === 'video' ? ($data['video_url']     ?? null) : null,
            'external_link'     => $contentType === 'link'  ? ($data['external_link'] ?? null) : null,
            'estimated_minutes' => !empty($data['estimated_minutes']) ? (int)$data['estimated_minutes'] : null,
            'sort_order'        => !empty($data['sort_order']) ? (int)$data['sort_order'] : 0,
            'is_published'      => $nowPublish ? 1 : 0,
        ];

        if ($nowPublish && !$wasPublished) {
            $updateData['published_by'] = $this->userId();
            $updateData['published_at'] = date('Y-m-d H:i:s');
        } elseif (!$nowPublish) {
            $updateData['published_by'] = null;
            $updateData['published_at'] = null;
        }

        $this->mat->update($id, $updateData);

        // Slide Gambar: tambah gambar baru yang diupload (jika ada)
        if ($contentType === 'slide') {
            $this->_handleSlideUploads($id);
        }

        return redirect()->to('/admin/materials/' . $material['subject_id'])
                         ->with('success', 'Berhasil diperbarui.');
    }

    // =========================================================
    // SLIDE — upload beberapa gambar untuk materi tipe 'slide'
    // =========================================================

    /**
     * Simpan gambar-gambar slide yang diupload (field: slide_images[]).
     * Ditambahkan ke urutan setelah slide yang sudah ada.
     * Menerima jpg/jpeg/png/webp/gif, maks 5 MB per gambar.
     */
    private function _handleSlideUploads(int $materialId): void
    {
        $files = $this->request->getFileMultiple('slide_images');
        if (empty($files)) {
            return;
        }

        $uploadDir = FCPATH . 'uploads/materials/slides/';
        if (!is_dir($uploadDir)) {
            mkdir($uploadDir, 0755, true);
        }

        // Urutan awal = jumlah slide yang sudah ada
        $startOrder = (int) $this->db->table('subject_material_slides')
            ->where('material_id', $materialId)
            ->countAllResults();

        $allowed = ['image/jpeg', 'image/png', 'image/webp', 'image/gif'];
        $captions = (array) ($this->request->getPost('slide_captions') ?? []);
        $i = 0;

        foreach ($files as $file) {
            if (!$file || !$file->isValid() || $file->hasMoved()) {
                continue;
            }
            if (!in_array($file->getMimeType(), $allowed, true)) {
                continue; // lewati file non-gambar
            }
            if ($file->getSize() > 5 * 1024 * 1024) {
                continue; // lewati gambar > 5MB
            }

            $newName = $file->getRandomName();
            $file->move($uploadDir, $newName);

            $this->db->table('subject_material_slides')->insert([
                'material_id' => $materialId,
                'image_path'  => $newName,
                'caption'     => isset($captions[$i]) ? trim((string) $captions[$i]) : null,
                'sort_order'  => $startOrder + $i,
                'created_at'  => date('Y-m-d H:i:s'),
            ]);
            $i++;
        }
    }

    // =========================================================
    // SLIDE — hapus satu gambar slide (AJAX POST)
    // POST /admin/materials/slide-delete/{slideId}
    // =========================================================
    public function slideDelete(int $slideId)
    {
        if (!$this->canManage()) {
            return $this->response->setJSON(['success' => false, 'message' => 'Akses hanya lihat.']);
        }
        $slide = $this->db->table('subject_material_slides')->where('id', $slideId)->get()->getRowArray();
        if (!$slide) {
            return $this->response->setJSON(['success' => false, 'message' => 'Slide tidak ditemukan.']);
        }

        // Hapus file fisik
        $path = FCPATH . 'uploads/materials/slides/' . $slide['image_path'];
        if (!empty($slide['image_path']) && is_file($path)) {
            @unlink($path);
        }

        $this->db->table('subject_material_slides')->where('id', $slideId)->delete();

        return $this->response->setJSON(['success' => true, 'message' => 'Slide dihapus.']);
    }

    // =========================================================
    // SLIDE — simpan urutan slide (AJAX POST)
    // POST /admin/materials/slide-reorder  body: order[]=slideId,...
    // =========================================================
    public function slideReorder()
    {
        if (!$this->canManage()) {
            return $this->response->setJSON(['success' => false, 'message' => 'Akses hanya lihat.']);
        }
        $order = (array) ($this->request->getPost('order') ?? []);
        $pos = 0;
        foreach ($order as $slideId) {
            $slideId = (int) $slideId;
            if ($slideId <= 0) continue;
            $this->db->table('subject_material_slides')
                ->where('id', $slideId)
                ->update(['sort_order' => $pos]);
            $pos++;
        }
        return $this->response->setJSON(['success' => true]);
    }

    // =========================================================
    // DELETE (AJAX POST)
    // =========================================================
    public function delete(int $id)
    {
        if (!$this->canManage()) {
            return $this->response->setJSON(['status' => 'error', 'message' => 'Akses hanya lihat.']);
        }
        $material = $this->mat->find($id);
        if (!$material) {
            return $this->response->setJSON(['status' => 'error', 'message' => 'Tidak ditemukan.']);
        }

        // Hapus children + publish records + forum threads
        $children = $this->mat->getAllChildren($id);
        foreach ($children as $child) {
            $this->_deleteSubMat($child['id']);
        }
        $this->_deleteSubMat($id);

        // Hapus file PDF parent jika ada
        if (!empty($material['file_path']) &&
            file_exists(FCPATH . 'uploads/materials/' . $material['file_path'])) {
            @unlink(FCPATH . 'uploads/materials/' . $material['file_path']);
        }
        $this->mat->delete($id);

        return $this->response->setJSON([
            'status'    => 'success',
            'message'   => 'Berhasil dihapus.',
            'subjectId' => $material['subject_id'],
        ]);
    }

    // =========================================================
    // PUBLISH PAGE — kelola publikasi sub materi ke kelas
    // GET /admin/materials/publish/{subMatId}
    // =========================================================
    public function publishPage(int $subMatId)
    {
        $subMat = $this->mat->find($subMatId);
        if (!$subMat || empty($subMat['parent_id'])) {
            return redirect()->to('admin/materials')->with('error', 'Hanya sub materi yang bisa dipublish.');
        }

        $year   = $this->activeYear();
        $yearId = (int)($year['id'] ?? 0);

        // Semua kelas yang level-nya sama dan mapelnya sama
        // Guru non-admin hanya lihat kelas yang dia ampu
        $teacherId = $this->isAdmin() ? null : $this->teacherId();
        $classes = $this->mat->getClassesForPublish(
            (int)$subMat['subject_id'],
            (int)$subMat['level'],
            $yearId,
            $teacherId
        );

        // Status publish per kelas
        $publishStatus = $this->mat->getPublishStatusForClasses($subMatId, $classes);

        $parent  = $this->mat->find($subMat['parent_id']);
        $subject = $this->subjectModel->find($subMat['subject_id']);

        return view('admin/materials/publish', [
            'title'         => 'Publikasi: ' . esc($subMat['title']),
            'subMat'        => $subMat,
            'parent'        => $parent,
            'subject'       => $subject,
            'classes'       => $classes,
            'publishStatus' => $publishStatus,
            'activeYear'    => $year,
        ]);
    }

    // =========================================================
    // PUBLISH SAVE — simpan perubahan publikasi (AJAX POST)
    // POST /admin/materials/publish/{subMatId}
    // =========================================================
    public function publishSave(int $subMatId)
    {
        if (!$this->canManage()) {
            return $this->response->setJSON(['success' => false, 'message' => 'Akses hanya lihat.']);
        }
        $subMat = $this->mat->find($subMatId);
        if (!$subMat || empty($subMat['parent_id'])) {
            return $this->response->setJSON(['success' => false, 'message' => 'Sub materi tidak valid.']);
        }

        $year   = $this->activeYear();
        $yearId = (int)($year['id'] ?? 0);

        // classIds yang dicentang user
        $checkedClassIds = array_map('intval',
            (array)($this->request->getPost('class_ids') ?? [])
        );

        // Jadwal publish per kelas: schedules[class_id] = 'Y-m-d\TH:i' (datetime-local)
        // Kosong = tampil segera. Nilai di masa depan = terjadwal.
        $schedules = (array)($this->request->getPost('schedules') ?? []);

        // Semua kelas yang relevan untuk mapel+level ini
        // Guru non-admin hanya kelola kelas yang dia ampu
        $teacherId2 = $this->isAdmin() ? null : $this->teacherId();
        $allClasses = $this->mat->getClassesForPublish(
            (int)$subMat['subject_id'],
            (int)$subMat['level'],
            $yearId,
            $teacherId2
        );
        $allClassIds = array_column($allClasses, 'id');

        // Publish ke kelas yang dicentang (dengan jadwal), unpublish dari yang tidak.
        // Pisahkan kelas yang tampil-segera dari yang terjadwal ke depan agar
        // notifikasi hanya dikirim untuk yang benar-benar sudah tampil.
        $now              = time();
        $notifyClassIds   = [];   // kelas yang materinya sudah tampil sekarang
        foreach ($allClassIds as $cid) {
            if (in_array($cid, $checkedClassIds)) {
                $rawSched   = isset($schedules[$cid]) ? trim((string) $schedules[$cid]) : '';
                $schedValue = $rawSched !== '' ? $rawSched : null;

                $this->mat->publishToClass($subMatId, $cid, $this->userId(), $yearId, $schedValue);

                // Tentukan apakah materi tampil segera untuk kelas ini
                $ts = $schedValue !== null ? strtotime(str_replace('T', ' ', $schedValue)) : false;
                if ($schedValue === null || ($ts !== false && $ts <= $now)) {
                    $notifyClassIds[] = $cid;
                }
            } else {
                $this->mat->unpublishFromClass($subMatId, $cid);
            }
        }

        // Kirim notifikasi hanya ke kelas yang materinya sudah tampil (bukan terjadwal ke depan)
        if (!empty($notifyClassIds)) {
            $this->_notifyPublish($subMatId, $subMat, $notifyClassIds);
        }

        $scheduledCount = count($checkedClassIds) - count($notifyClassIds);
        $msg = 'Pengaturan publikasi berhasil disimpan.';
        if ($scheduledCount > 0) {
            $msg .= ' ' . $scheduledCount . ' kelas dijadwalkan tampil nanti.';
        }

        return $this->response->setJSON([
            'success' => true,
            'message' => $msg,
        ]);
    }

    // =========================================================
    // USE PAGE — daftar sub materi level yang sama untuk digunakan
    // GET /admin/materials/use/{subjectId}
    // =========================================================
    public function usePage(int $subjectId)
    {
        if ($r = $this->denyIfReadonly()) return $r;
        $subject = $this->subjectModel->find($subjectId);
        if (!$subject) return redirect()->to('admin/materials')->with('error', 'Mapel tidak ditemukan.');

        $year   = $this->activeYear();
        $yearId = (int)($year['id'] ?? 0);
        $userId = $this->userId();

        // Level-level yang diajar guru ini
        $levels = $this->getLevelsForSubject($subjectId, $yearId);
        sort($levels);

        // Sub materi tersedia per level
        $available = [];
        foreach ($levels as $lv) {
            $items = $this->mat->getAvailableForTeacher($subjectId, $lv, $yearId, $userId);
            if (!empty($items)) {
                $available[$lv] = $items;
            }
        }

        // Kelas yang diajar guru ini per level
        $classesByLevel = [];
        $tidForUse      = $this->isAdmin() ? null : $this->teacherId();
        foreach ($levels as $lv) {
            $classesByLevel[$lv] = $this->mat->getClassesForPublish($subjectId, $lv, $yearId, $tidForUse);
        }

        return view('admin/materials/use', [
            'title'         => 'Gunakan Materi: ' . esc($subject['name']),
            'subject'       => $subject,
            'available'     => $available,
            'classesByLevel'=> $classesByLevel,
            'activeYear'    => $year,
        ]);
    }

    // =========================================================
    // USE SAVE — simpan penggunaan sub materi (AJAX POST)
    // POST /admin/materials/use
    // =========================================================
    public function useSave()
    {
        if (!$this->canManage()) {
            return $this->response->setJSON(['success' => false, 'message' => 'Akses hanya lihat.']);
        }
        $subMatId  = (int)$this->request->getPost('sub_mat_id');
        $classIds  = array_map('intval',
            (array)($this->request->getPost('class_ids') ?? [])
        );

        if (!$subMatId || empty($classIds)) {
            return $this->response->setJSON(['success' => false, 'message' => 'Data tidak lengkap.']);
        }

        $year = $this->activeYear();

        $ok = $this->mat->useSubMaterial($subMatId, $classIds, $this->userId(), (int)($year['id'] ?? 0));

        if ($ok) {
            $sub = $this->mat->find($subMatId);
            $this->_notifyPublish($subMatId, $sub ?? [], $classIds);
        }

        return $this->response->setJSON([
            'success' => $ok,
            'message' => $ok ? 'Sub materi berhasil digunakan.' : 'Gagal menggunakan sub materi.',
        ]);
    }

    // =========================================================
    // UPLOAD IMAGE — CKEditor Simple Upload Adapter
    // POST /admin/materials/upload-image
    // =========================================================
    public function uploadImage()
    {
        if (!$this->request->isAJAX()) {
            return $this->response->setStatusCode(403)->setJSON(['error' => ['message' => 'Forbidden']]);
        }
        if (!$this->canManage()) {
            return $this->response->setStatusCode(403)->setJSON(['error' => ['message' => 'Akses hanya lihat.']]);
        }

        $file = $this->request->getFile('upload');
        if (!$file || !$file->isValid()) {
            return $this->response->setJSON(['error' => ['message' => 'File tidak valid.']]);
        }

        $allowed = ['image/jpeg', 'image/jpg', 'image/png', 'image/gif', 'image/webp'];
        if (!in_array($file->getMimeType(), $allowed)) {
            return $this->response->setJSON(['error' => ['message' => 'Hanya file gambar yang diizinkan (jpg, png, gif, webp).']]);
        }

        if ($file->getSize() > 5 * 1024 * 1024) {
            return $this->response->setJSON(['error' => ['message' => 'Ukuran maksimal 5 MB.']]);
        }

        $uploadDir = FCPATH . 'uploads/materials/images/';
        if (!is_dir($uploadDir)) {
            mkdir($uploadDir, 0755, true);
        }

        $newName = $file->getRandomName();
        $file->move($uploadDir, $newName);

        // CKEditor 5 Simple Upload Adapter mengharapkan format: { "url": "..." }
        return $this->response->setJSON([
            'url' => base_url('uploads/materials/images/' . $newName),
        ]);
    }

    // =========================================================
    // PROGRESS
    // =========================================================
    public function progress(int $subjectId)
    {
        $subject = $this->subjectModel->find($subjectId);
        $year    = $this->activeYear();
        $yearId  = (int)($year['id'] ?? 0);

        $tidForProg = $this->isAdmin() ? null : $this->teacherId();
        $classes = $this->mat->getClassesForPublish($subjectId, 0, $yearId, $tidForProg);
        $selectedClassId = $this->request->getGet('class_id') ?: ($classes[0]['id'] ?? null);
        $materials = $summary = [];

        if ($selectedClassId) {
            // Filter per mapel agar daftar progres tidak bercampur antar mapel
            $materials = $this->mat->getPublishedByClass((int)$selectedClassId, $yearId, $subjectId);
            $summary   = $this->prog->getSummaryByClass((int)$selectedClassId, $yearId);
        }

        return view('admin/materials/progress', [
            'title'           => 'Progress: ' . ($subject['name'] ?? ''),
            'subject'         => $subject,
            'classes'         => $classes,
            'selectedClassId' => (int)$selectedClassId,
            'materials'       => $materials,
            'summary'         => $summary,
            'activeYear'      => $year,
        ]);
    }

    public function progressDetail(int $materialId)
    {
        $material = $this->mat->find($materialId);
        if (!$material) return redirect()->to('admin/materials')->with('error', 'Sub materi tidak ditemukan.');

        $subject  = $this->subjectModel->find($material['subject_id'] ?? 0);
        $year     = $this->activeYear();
        $yearId   = (int)($year['id'] ?? 0);

        // Ambil class_id dari query param; jika tidak ada, ambil kelas pertama yang punya publish record
        $classId = $this->request->getGet('class_id');
        if (!$classId) {
            $firstPublish = $this->db->table('subject_material_publishes')
                ->select('class_id')
                ->where('material_id', $materialId)
                ->where('is_active', 1)
                ->orderBy('class_id', 'ASC')
                ->limit(1)
                ->get()->getRowArray();
            $classId = $firstPublish['class_id'] ?? null;
        }

        // Semua kelas yang punya publish record untuk sub materi ini (untuk tab/filter kelas)
        $publishedClasses = $this->db->table('subject_material_publishes smp')
            ->select('c.id, c.name')
            ->join('classes c', 'c.id = smp.class_id')
            ->where('smp.material_id', $materialId)
            ->where('smp.is_active', 1)
            ->orderBy('c.name', 'ASC')
            ->get()->getResultArray();

        $students = $classId
            ? $this->prog->getDetailByMaterial($materialId, (int)$classId, $yearId)
            : [];

        // back_url: dari show/{id} atau dari progress/{subjectId}
        $backUrl = $this->request->getGet('back_url')
            ?: site_url('admin/materials/progress/' . ($subject['id'] ?? '') . '?class_id=' . $classId);

        return view('admin/materials/progress_detail', [
            'title'            => 'Detail Progress: ' . ($material['title'] ?? ''),
            'material'         => $material,
            'subject'          => $subject,
            'students'         => $students,
            'classId'          => $classId,
            'publishedClasses' => $publishedClasses,
            'backUrl'          => $backUrl,
        ]);
    }

    // ── Private helpers ───────────────────────────────────────────────────

    private function _deleteSubMat(int $id): void
    {
        $sub = $this->mat->find($id);
        if ($sub && !empty($sub['file_path']) &&
            file_exists(FCPATH . 'uploads/materials/' . $sub['file_path'])) {
            @unlink(FCPATH . 'uploads/materials/' . $sub['file_path']);
        }

        // Hapus gambar slide (file fisik + baris)
        $slides = $this->db->table('subject_material_slides')->where('material_id', $id)->get()->getResultArray();
        foreach ($slides as $sl) {
            $p = FCPATH . 'uploads/materials/slides/' . $sl['image_path'];
            if (!empty($sl['image_path']) && is_file($p)) {
                @unlink($p);
            }
        }
        $this->db->table('subject_material_slides')->where('material_id', $id)->delete();

        $this->db->table('material_progress')->where('material_id', $id)->delete();
        $this->db->table('subject_material_publishes')->where('material_id', $id)->delete();
        $this->db->table('forum_threads')
            ->where('related_type', 'material')
            ->where('related_id', $id)
            ->where('is_system', 1)
            ->delete();
    }

    private function _notifyPublish(int $subMatId, array $subMat, array $classIds): void
    {
        try {
            $subjectName = $this->db->table('subjects')
                ->select('name')
                ->where('id', $subMat['subject_id'] ?? 0)
                ->get()->getRowArray()['name'] ?? '';

            notify_classes(
                $classIds,
                'materi_baru',
                'Sub Materi Baru: ' . ($subMat['title'] ?? ''),
                "Mapel: {$subjectName}",
                base_url('siswa/belajar/sub/' . $subMatId),
                'material',
                $subMatId
            );
        } catch (\Throwable $e) {
            log_message('warning', '[Materials] Notif gagal: ' . $e->getMessage());
        }
    }
}
